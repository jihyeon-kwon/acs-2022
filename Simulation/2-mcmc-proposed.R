...
for (MM in 1:4) {
        filename = paste("proposed.", toString(MM), ".Rda", sep="")
        outfile = paste("proposed_log.",toString(MM),".txt",sep="")
        ...
        SimMCMC<-foreach(kk=1:25) %dorng% {
                ...
                code <- nimbleCode({
                        ...
                        for (t in 2010:2013) {
                                ...
                                for (i in 1:N) {
                                        for (p in 1:2) {
                                                ...
                                                X[(t-2010)*N+i, p+2] ~ dnorm(theta.t[((t-2010)+1), p] + g[(t-2010)*N+i, p], tau = tausq.i[i, p])
                                ...
                                for (p in 1:4) {
                                        ...
                                        for (l in 1:N1) {
                                                b[l, p, (t-2010+1)] <- oyInd[l]
                                                X1[(t-2010)*N1+l, p] ~ dnorm(X[(t-2010)*N+b[l, p, (t-2010+1)], p], sd = se1[(t-2010)*N1+l, p])
                                        }
                                }
                                ...
                                for (p in 1:2) {
                                        ...
                                        # NC 1-year estimates for socioeconomic
                                        NC1[(t-2010)+1, p+2] ~ dnorm(theta.t[((t-2010)+1), p], sd = NCse1[(t-2010)+1, p+2])
                        ...
                        for (t in 2014:2014) {
                                ...
                                for(i in 1:N) {
                                        # Data Layer
                                        y[(t-2014)*N+i] ~ dnorm(mean = (beta0 + inprod(X[(t-2010)*N+i, 1:4], beta[1:4])
                                                                        + f[(t-2014)*N+i]), 
                                                                var = sigmasq)
                                        ...
                                        for (p in 1:4) {
                                                ...
                                                avg5[(t-2014)*N+i, p] <- (1/5) * (X[(t-2014)*N+i, p] + X[(t-2013)*N+i, p] + X[(t-2012)*N+i, p] + X[(t-2011)*N+i, p] + X[(t-2010)*N+i, p])
                                                X5[(t-2010)*N+i, p] ~ dnorm(avg5[(t-2014)*N+i, p], sd = se5[(t-2010)*N+i, p])
                        ...
                        for (t in 2015:2023) {
                                ...
                                for (p in 1:4) {
                                        ...
                                        for (l in 1:N1) {
                                                b[l, p, (t-2010+1)] <- oyInd[l]
                                                X1[(t-2010)*N1+l, p] ~ dnorm(X[(t-2010)*N+b[l, p, (t-2014+1)], p], sd = se1[(t-2010)*N1+l, p])
                                        }
                                }
                ...
                constants <- list(N = N, N1 = N1, oyInd = oyInd, adj = adj, num = num,
                                  se5 = as.matrix(se5), se1 = as.matrix(se1), 
                                  NCse1 = as.matrix(NCse1))
                ...
                inits <- list(beta0 = 0, beta = c(rep(0, 4)), 
                              ...
                              theta.t = matrix(0, nrow = 2023-2010+1, ncol = 2),
                              tausq.i = matrix(1, nrow = N, ncol = 2),
                              tausq.t = matrix(1, nrow = 2023-2010+1, ncol = 2),
                              f = rep(0, N*10), u = rep(0, N*10),
                              g = matrix(0, nrow = N*(2023-2010+1), ncol = 2),
