...
######################################################
# Simulate Explanatory Variables : Skewed Covariates #
######################################################
...
# We actually use data from 5-18 but created 18 years for convienience.
...
# histogram of var2 before making the data skewed
hist(unlist(SimulatedData[which(SimulatedData$time==18), "var2"]),
     main = "histogram for var2 (demographic)")

# make var 2 skewed (to mimic %Black pop in NC)
# randomly select 10 locations
skew.location <- sample(1:100, 10)

# randomly choose three counties that will have the most extreme value
skew.location2 <- sample(skew.location, 3)

# randomly choose three counties that will have a more extreme values
skew.location3 <- sample(skew.location, 3)

# skew the data
SimulatedData <- SimulatedData %>%
        mutate(var2 = case_when(location %in% skew.location2 ~ var2 - 2.5,
                                location %in% skew.location3 ~ var2 - 1,
                                location %in% skew.location ~ var2 - 0.5,
                                TRUE ~ var2))

# check the histogram to see if it's skewed 
hist(unlist(SimulatedData[which(SimulatedData$time==18), "var2"]),
     main = "histogram for var2 (demographic)")
...
# check the histogram to see if it's skewed 
hist(unlist(SimulatedData[which(SimulatedData$time==5), "var2"]),
     main = "histogram for var2 (demographic)")
...
save(N, N1, oyInd, adj, num,
     ...
     y, X5, X1, NC1, X,
     plot_data,
     file = here("Simulation/additional-simul/simul-data-skewed.Rda"))
