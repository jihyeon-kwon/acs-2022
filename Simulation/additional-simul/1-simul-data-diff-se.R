...
#########################################################################
# Simulate Explanatory Variables : Different SE values for generating Y #
#########################################################################
...
for (m in 1:M){
        y.sim[, m+c] <- rnorm(n*(T+4), 
                              ...
                              + y.sim$f.RE,
                              sd = 0.8) # changed this from 0.1 to 0.8
}
#######################
# Save Simulated Data #
#######################
...
save(N, N1, oyInd, adj, num,
     ...
     y, X5, X1, NC1, X,
     plot_data,
     file = here("Simulation/additional-simul/simul-data-diff-se.Rda"))
